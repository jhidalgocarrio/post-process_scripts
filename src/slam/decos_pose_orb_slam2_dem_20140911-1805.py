...
path_orb_slam2_orientation_file = path + 'pose_orb_slam2_orientation.0.data'
...
def odometry_error_bar(fig_num, info, pred_mean, resampling_time='1s', color_bar='Reds'):
    ########################
    # Plot Bar 
    ########################
    matplotlib.rcParams.update({'font.size': 15, 'font.weight': 'bold'})
...
# ORB_SLAM2 Orientation (in quaternion form)
orb_slam2_orient = data.QuaternionData()
orb_slam2_orient.readData(os.path.expanduser(path_orb_slam2_orientation_file), cov=True)
...
##########################################################################
slam_position = np.column_stack((orb_slam2_position.x.values, orb_slam2_position.y.values,  orb_slam2_position.z.values ))
