...
#Localization
localization = data.ThreeData()
localization.readData(localization_file, cov=True)
localization.eigenValues()

#ExoTeR wheel Odometry
wheel_odo = data.ThreeData()
wheel_odo.readData(wheel_odometry_file, cov=True)
wheel_odo.eigenValues()

#Visual Odometry
visual_odo = data.ThreeData()
visual_odo.readData(visual_odometry_file, cov=False)
visual_odo.eigenValues()
...
xposition = wheel_odo.getAxis(0)[0::50]
yposition = wheel_odo.getAxis(1)[0::50]
ax.plot(xposition, yposition, marker='s', linestyle='-', label="Localization Position", color=[0.1,0,0.8], lw=2)

xposition = wheel_odo.getAxis(0)[0::50]
yposition = wheel_odo.getAxis(1)[0::50]
ax.plot(xposition, yposition, marker='o', linestyle='-.', label="Jacobian Odometry", color=[0.3,0.2,0.4], lw=2)

xposition = wheel_odo.getAxis(0)[0::50]# reduce number of points
yposition = wheel_odo.getAxis(1)[0::50]
xycov = wheel_odo.getCov(1)[0::10]
...
xposition = visual_odo.getAxis(0)[0::2]
yposition = visual_odo.getAxis(1)[0::2]
ax.plot(xposition, yposition, marker='x', linestyle='--', label="Visual Odometry", color=[0,0.5,1], lw=2)
...
xposition = wheel_odo.getAxis(0)[0::50]
yposition = wheel_odo.getAxis(1)[0::50]
zposition = wheel_odo.getAxis(2)[0::50]
ax.plot(xposition, yposition, zposition, marker='o', linestyle='-.', label="Jacobian Odometry", color=[0.3,0.2,0.4], lw=2)

xposition = visual_odo.getAxis(0)[0::50]
yposition = visual_odo.getAxis(1)[0::50]
zposition = visual_odo.getAxis(2)[0::50]
