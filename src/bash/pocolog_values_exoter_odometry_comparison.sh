...
if [ $# -ge $NUM_ARGUMENTS ]; then
    ...
    pocolog ${args[0]}/robot_odometry.${args[1]}.log -s /robot_odometry.pose_samples_out --field time,position,cov_position > pose_odo_position.${args[1]}.data
    pocolog ${args[0]}/robot_odometry.${args[1]}.log -s /robot_odometry.pose_samples_out --field time,orientation,cov_orientation > pose_odo_orientation.${args[1]}.data
    echo 'Pose Odometry Samples... [DONE]'

    pocolog ${args[0]}/robot_odometry.${args[1]}.log -s /robot_odometry.delta_pose_samples_out --field time,position,cov_position > delta_pose_odo_position.${args[1]}.data
    pocolog ${args[0]}/robot_odometry.${args[1]}.log -s /robot_odometry.delta_pose_samples_out --field time,orientation,cov_orientation > delta_pose_odo_orientation.${args[1]}.data
